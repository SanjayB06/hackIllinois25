          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {step === 1 && (
              <>
                ...
                <Button type="button" onClick={nextStep} className="w-full bg-sage-600 hover:bg-sage-700 text-white">
                ...
                <div className="space-y-4">
                  ...
                  >
                    {["omnivore", "vegetarian", "vegan"].map((diet) => (
                      <div key={diet} className="flex items-center space-x-2 bg-sage-100 rounded-lg p-4 cursor-pointer">
                        <RadioGroupItem value={diet} id={diet} />
                        <Label htmlFor={diet} className="capitalize cursor-pointer text-sage-800">
                          {diet}
                        </Label>
                      </div>
                    ))}
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {Object.keys(formData.allergies).map((allergy) => (
                      <div key={allergy} className="flex items-center space-x-2 bg-sage-100 rounded-lg p-4">
                        <Checkbox
                          id={allergy}
                          checked={formData.allergies[allergy as keyof typeof formData.allergies]}
                          onCheckedChange={(checked) => handleAllergyChange(allergy, checked as boolean)}
                        />
                        <Label htmlFor={allergy} className="capitalize cursor-pointer text-sage-800">
                          {allergy}-free
                        </Label>
                      </div>
                    ))}
                ...
                <div className="space-y-4">
                  ...
                  >
                    {["small", "medium", "large"].map((size) => (
                      <div key={size} className="flex items-center space-x-2 bg-sage-100 rounded-lg p-4 cursor-pointer">
                        <RadioGroupItem value={size} id={size} />
                        <Label htmlFor={size} className="capitalize cursor-pointer text-sage-800">
                          {size}
                        </Label>
                      </div>
                    ))}
